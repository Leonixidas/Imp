	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					IMP_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					IMP_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define HZ_FUNC_SIG __func__
#else
#define HZ_FUNC_SIG "IMP_FUNC_SIG unknown!"
#endif
...
#else
#define IMP_PROFILE_BEGIN_SESSION(name, filepath)
#define IMP_PROFILE_END_SESSION()
#define IMP_PROFILE_SCOPE(name)
#define IMP_PROFILE_FUNCTION()
#endif